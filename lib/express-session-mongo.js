...
var MongoStore = function(options) {
    ...
    // Configurable bson parser.  defaults to true.
    if(options.native_parser === undefined) {
      this.native_parser = true;
    } else {
      this.native_parser = options.native_parser;
    }
    ...
    this._db = new Db( dbName, server, { native_parser: this.native_parser });
